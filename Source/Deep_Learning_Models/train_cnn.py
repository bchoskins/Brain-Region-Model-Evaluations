...
## input layer
    ...
    model.compile(loss='mean_squared_error', metrics=['mean_squared_error', 'mean_absolute_error'], optimizer=adam)     
    return model
...
es = keras.callbacks.EarlyStopping(monitor='val_loss',
                    ...
                    patience=5,
                    verbose=0, mode='auto')
cp = keras.callbacks.ModelCheckpoint(filepath='/Dedicated/jmichaelson-wdata/lbrueggeman/ukbb_sex/trained_models/2019-10-21-cnn-set{}_sexresid_LRe5'.format(batch_num),
                                verbose=1,
                                save_best_only=True)
...
history = mod.fit(x=x[train_ind],
          y=sex_resid[train_ind],
          validation_data = [x[val_ind], sex_resid[val_ind]],
          batch_size=32,
          epochs=50,
          verbose=1,
         shuffle=False,
...
pd.DataFrame(history.history).to_csv(
    filepath='/Dedicated/jmichaelson-wdata/lbrueggeman/ukbb_sex/trained_models/2019-10-21-cnn-set{}_sexresid_LR1e-5.tsv'.format(batch_num),
    index=False, sep='\t')
